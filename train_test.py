...
from virtual_data import VIRTUAL
...
def run(config_file=None):
    ...
    for dataset_idx, dataset in enumerate(model_configs['exp_opts']['datasets']):
        ...
        if configs['model_opts']['dataset'] == 'pie':
            ...
            imdb = PIE(data_path=r'G:\BJ\pedestrian\PIE')
        elif configs['model_opts']['dataset'] == 'jaad':
            # if use docker:
            # imdb = JAAD(data_path=os.environ.copy()['JAAD_PATH'])

            # if use local path
            imdb = JAAD(data_path=r'G:\BJ\pedestrian\JAAD')
        elif configs['model_opts']['dataset'] == 'virtual':

            imdb = VIRTUAL(data_path=r'F:\Virtual_PedCross\data1')
...
if __name__ == '__main__':
    ...
    config_file = r'config_files/student.yaml'
    model_name = None
    dataset = None
